$(document).ready(function() {
    ...
    var totalData = $("#totalAvailableRecords").val();
	var sorting = getCheckboxValues('sorting');
	$.ajax({
		type: 'POST',
		url : "load-cars.php",
		dataType: "json",			
		data:{totalRecord:totalRecord, car_type:car_type, car_seat_capacity:car_seat_capacity, car_transmission:car_transmission, location:location, sorting:sorting},
        ...
        if(scrollHeight == $(document).height()){	
            if(totalRecord <= totalData){
				...
				$.ajax({
					...
					url : "load-cars.php",
					dataType: "json",			
					data:{totalRecord:totalRecord, car_type:car_type, car_seat_capacity:car_seat_capacity, car_transmission:car_transmission, location:location},
					success: function (data) {
						$("#results").append(data.products);
						$('.loader').hide();
						totalRecord++;
